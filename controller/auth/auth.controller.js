...
const login = async (req, res, next) => {
    ...
    try {
        ...
        crypto.pbkdf2(password, User_.salt, 310000, 32, 'sha256', async (err, hashPassword) => {
            ...
            const token = jwt.sign({ id: User_.user_id }, process.env.JWT_SECRET, { algorithm: 'HS256', expiresIn: "1h"})
...
const refreshAndVerifyToken = async (req, res) => {
    ...
    try {
        ...
        const newAccessToken = jwt.sign({ id: user.id }, process.env.JWT_SECRET, { algorithm: 'HS256', expiresIn: "1h"})
